...
if (apiid != None and apihash!= None and bottoken != None):
    try:
        Mafia = TelegramClient('Mafia', apiid, apihash).start(bot_token=bottoken)
...
async def get_user_join(id):
    ok = True
    try:
        await Mafia(GetParticipantRequest(channel=channel, user_id=id))
        ok = True
...
async def _(event):
    ...
    if event.user_joined or event.user_added:
        ...
        pp = await Mafia.get_participants(chat)
        count = len(pp)
        ...
        if x is True:
            ...
            await Mafia.edit_permissions(event.chat.id, user.id, until_date=None, send_messages=False)
        
        await event.reply(msg, buttons=butt)


@Mafia.on(events.NewMessage(incoming=True))
async def mute_on_msg(event):
    ...
    temp = await Mafia(GetFullUserRequest(event.sender_id))
    if x is False:
        if temp.user.bot:
            return
        nm = temp.user.first_name
        try:
            await Mafia.edit_permissions(event.chat.id, event.sender_id, until_date=None, send_messages=False)
        except Exception as e:
            print(str(e))
            return
...
@Mafia.on(events.callbackquery.CallbackQuery(data=re.compile(b"unmute_(.*)")))
async def _(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    if uid == event.sender_id:
        x = await get_user_join(uid)
        nm = (await Mafia(GetFullUserRequest(uid))).user.first_name
        if x is False:
            await event.answer(f"You haven't joined @{channel} yet!", cache_time=0, alert=True)
        elif x is True:
            try:
                await Mafia.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
            except Exception as e:
                print(str(e))
                return
